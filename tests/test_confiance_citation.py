...
def test_score_correspondance_moyenne() -> None:
    assert score_correspondance([_ev(0.4), _ev(0.6), _ev(None)]) == 0.5
    assert score_correspondance([_ev(None), _ev(None)]) is None
    assert score_correspondance([]) is None
...
def test_pas_de_citation_step_confiance_inchangee() -> None:
    out = confiance_apres_citation(NiveauConfiance.ELEVE, None)
    assert out is NiveauConfiance.ELEVE


def test_aucune_douteuse_confiance_inchangee() -> None:
    out = confiance_apres_citation(NiveauConfiance.ELEVE, _res(5, 0))
    assert out is NiveauConfiance.ELEVE


def test_toutes_douteuses_force_incertain() -> None:
    out = confiance_apres_citation(NiveauConfiance.ELEVE, _res(0, 3))
...
def test_minorite_de_douteuses_ne_change_rien() -> None:
    ...
    assert confiance_apres_citation(NiveauConfiance.MOYEN, _res(4, 1)) is (
        NiveauConfiance.MOYEN
    )


def test_majorite_de_douteuses_descend_un_cran() -> None:
    assert confiance_apres_citation(NiveauConfiance.ELEVE, _res(1, 3)) is (
        NiveauConfiance.MOYEN
    )
    assert confiance_apres_citation(NiveauConfiance.MOYEN, _res(1, 3)) is (
        NiveauConfiance.FAIBLE
    )


def test_plancher_incertain() -> None:
    out = confiance_apres_citation(NiveauConfiance.INCERTAIN, _res(1, 3))
    assert out is NiveauConfiance.INCERTAIN
