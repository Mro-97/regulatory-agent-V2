...
class DocumentReglementaire(BaseModel):
    ...
    url_source: Optional[str] = Field(default=None, description="URL canonique sur le site source.")
    ...
    repeal_date: Optional[date] = Field(default=None, description="Date d'abrogation. None = en vigueur.")
    ...
    chapitres: list[Chapitre] = Field(default_factory=list, description="Structure du document.")
    textes_lies: list[TexteLie] = Field(default_factory=list, description="Relations avec d'autres textes.")
    hash_document: Optional[str] = Field(default=None, description="Hash SHA-256 du document.")
    date_indexation: Optional[datetime] = Field(default=None, description="Horodatage UTC d'indexation Qdrant.")
...
class MetadonneesChunk(BaseModel):
    ...
    position_dans_article: int = Field(default=0, ge=0, description="Indice du chunk dans l'article.")
...
class EnregistrementAudit(BaseModel):
    ...
    hash_precedent: Optional[str] = Field(default=None, description="Hash du record précédent.")
...
class RequeteQuestion(BaseModel):
    ...
    question: str = Field(..., min_length=3, description="Question réglementaire.")
    date_contexte: Optional[date] = Field(default=None, description="Date de contexte réglementaire.")
