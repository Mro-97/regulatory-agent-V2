---
name: regulatory-agent-architecture
description: Use when designing, modifying, reviewing, or debugging the overall Regulatory Agent V2 architecture, service boundaries, component responsibilities, memory budget, inter-module communication, or core project structure.
---

# regulatory-agent-architecture

## Responsibilities

- Maintain a coherent single-machine architecture on `m4pro2` (M4 Pro, 24 GB).
- Map every service and agent to its port and its memory budget.
- Keep gateway/orchestration separate from retrieval, expert analysis, storage and monitoring.
- Define clear interfaces between services.
- Protect the memory budget: 24 GB are shared by the models, Qdrant, PostgreSQL, Redis and the OS. Only one model is resident at a time.
- Treat DeepSeek-R1 14B as an occasional workload (8-10 GB in 4-bit): load on demand, unload immediately after.
- Keep every service bound to `127.0.0.1`.
- Preserve the current project structure unless a migration is justified.

## Decision procedure

1. Identify the affected component and its owning module.
2. Identify upstream/downstream contracts.
3. Check memory and runtime constraints.
4. Check data ownership: Qdrant, PostgreSQL, Redis and JSON files.
5. Check audit and human-validation implications.
6. Propose the smallest coherent change.
7. Define tests and migration/rollback steps.

## Do not

- Reintroduce a distributed deployment, or assume a second machine exists.
- Keep several heavy models resident at the same time.
- Bind a service to `0.0.0.0`.
- Introduce hidden cloud dependencies.
- Create duplicate sources of truth for regulatory metadata.
