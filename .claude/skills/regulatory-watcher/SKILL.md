...
description: Use when implementing or reviewing automated regulatory monitoring, source polling, change detection, alert generation, and the compliance radar workflow.
---
...
`source scheduler → fetch → normalize → hash/compare → classify change → create version → create alert → pending_alerts → human review → audit`

The current project plans a six-hour polling cadence.

## Change detection

Hashes are useful but not sufficient when formatting changes without legal substance or metadata changes independently. Compare normalized content and relevant metadata.

## Alerts

Every alert should identify source, document, previous version, detected change, detection timestamp, evidence, proposed severity and review status.

Watcher output is a proposal, not a legal conclusion. Route meaningful changes into `pending_alerts`.

## Operations

Jobs must be idempotent, avoid duplicate alerts, handle source downtime gracefully, avoid leaking secrets and keep scheduling separate from source-specific parsing.
